...
export const BlurImage = ({
  ...
  // When fill is used, strip width/height from style to avoid Next.js error
  const safeStyle = fill ? (() => {
    if (!style) return undefined;
    const { width: _w, height: _h, ...s } = style as Record<string, unknown>;
    return Object.keys(s).length > 0 ? s as React.CSSProperties : undefined;
  })() : style;
  return (
    <Image
      className={cn(
